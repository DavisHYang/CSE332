package paralleltasks;

import cse332.graph.GraphUtil;

import java.util.List;
import java.util.Map;
import java.util.concurrent.ForkJoinPool;
import java.util.concurrent.RecursiveAction;

public class RelaxOutTaskBad extends RecursiveAction {

    public static final ForkJoinPool pool = new ForkJoinPool();
    public static final int CUTOFF = 10;

    private final List<Map<Integer, Integer>> g;
    private final int[] dist, distCopy, pred;
    private final int hi, lo;

    public RelaxOutTaskBad(List<Map<Integer, Integer>> g, int[] dist, int[] pred, int[] distCopy, int lo, int hi) {
        this.g = g;
        this.dist = dist;
        this.distCopy = distCopy;
        this.pred = pred;
        this.hi = hi;
        this.lo = lo;
    }

    protected void compute() {
        if(hi - lo <= CUTOFF) {
            sequential(g, dist, pred, distCopy, lo, hi);
        } else {
            int mid = lo + (hi-lo)/2;

            RelaxOutTaskBad left = new RelaxOutTaskBad(g, dist, pred, distCopy, lo, mid);
            RelaxOutTaskBad right = new RelaxOutTaskBad(g, dist, pred, distCopy, mid, hi);

            left.fork();
            right.compute();
            left.join();
        }
    }

    public static void sequential(List<Map<Integer, Integer>> g, int[] dist, int[] pred, int[] distCopy, int lo, int hi) { // relax current range
        for(int i = lo; i < hi; i++) {
            for(int w = lo; w < hi; w++) { // each edge in range
                for(int key: g.get(w).keySet()) {
                    if(distCopy[w] != GraphUtil.INF && distCopy[w] + g.get(w).get(key) < dist[key]) {
                        dist[key] = distCopy[w] + g.get(w).get(key);
                        distCopy[key] = distCopy[w] + g.get(w).get(key);
                        pred[key] = w;
                    }
                }
            }
        }
    }

    public static void parallel(List<Map<Integer, Integer>> g, int[] dist, int[] pred, int[] distCopy) {
        pool.invoke(new RelaxOutTaskBad(g, dist, pred, distCopy, 0, g.size()));
    }
}
